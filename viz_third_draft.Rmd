...
This plot shows the major demographic changes that Chicago has experienced over the last several decades. The grey box shows the last 17 years, in which the white population has increased sharply, and minority populations have begun declining or leveled off. This time period is the area of focus for the rest of the graphs.
...
This next plot shows the demographic changes each neighborhood has experienced in the last 17 years. The lower right quadrant shows neighborhoods that have both increased their percentages of white residents and decreased their percentages of black residents. The graph also has an accompanying map showing the locations of those neighborhoods. That map also shows that the majority of neighborhoods in Chicago fit into this category.
...
p2 <- ggplot() +
    geom_polygon(data = temp_data[which(temp_data$highlight > 0),], 
                 aes(x = long, y = lat, group = group), fill = "#FF5416", alpha = .65) +
    geom_path(data = temp_data[which(temp_data$highlight > 0),], 
                 aes(x = long, y = lat, group = group), col = "grey", alpha = .2) +
    geom_polygon(data = temp_data[which(temp_data$highlight == 0),], 
                 aes(x = long, y = lat, group = group), fill = "#00B28F", alpha = .2) +
    geom_path(data = temp_data[which(temp_data$highlight == 0),], 
...
neighborhood_master_wide %>%
  ...
  mutate(YEAR = gsub("pwhite_1970_2000", "1970 - 2000", YEAR),
    ...
    coord_equal() +
    facet_wrap(~YEAR) +
    scale_fill_gradient2(low = "#00B28F", high = "#FF5416") +
...
p1 <- ggplot(data = plot_data, aes(x = YEAR, y = mean_rank, 
  ...
  geom_label(data = plot_data[plot_data$YEAR == 2017 &
          ...
          theme_classic() +
  guides(col = FALSE) +
   scale_color_manual(values = c("#00856B", "#00BF9A", "#C94211", "#FF5416")) +     
          scale_y_reverse(limits = c(125,0)) +
  annotate("label", label="Garfield Ridge", x=2017, y=100, col = "#00BF9A", family="Segoe UI Bold") +
...
p2 <- ggplot() +
    ...
    geom_sf(data = zillow_shp[zillow_shp$RegionID %in% changing_neigh,],
            ...
            col = "grey", alpha = .6) +
    scale_fill_manual(values = c("#00856B", "#00BF9A", "#C94211", "#FF5416")) +     
    theme_minimal() +
    guides(fill = FALSE) +
    coord_sf(datum=NA) +      
    theme(axis.line = element_blank(),
...
plot_data1[order(plot_data1$REGION, plot_data1$YEAR),] %>%
  ...
  annotate("label", label="Garfield Ridge", x=.0073, y=.0015, alpha = .6, family="Segoe UI Bold", col = "#00856B") +
...
This next plot shows changes in housing supply for neighborhoods between 2000-2017. Neighborhoods with increasing supply are downtown and in the Northwest of Chicago, while neighborhods in the south and west are mostly losing housing stock.

```{r Change in Housing Map, message=F, warning=F}

plot_data <- neighborhood_master_wide %>%
   mutate(hu_2017_2000 = `2017_hu` - `2000_hu`,
          cuts = cut(hu_2017_2000, 
                     breaks = c(-2000, -1000, 0, 1000, 2000, 3000, 4000, 14000),
                     ordered_result = T),
          levels = as.numeric(cuts) -3) %>%
    subset(subset = RegionID != "403159", select=c(long, lat, group, levels, RegionID, cuts))
...
ggplot(plot_data, aes(x = long, y = lat)) +
  geom_polygon(aes(fill = factor(levels),
       ...
       geom_path(aes(group = group), 
                  color = "grey", size = 0.01, alpha = 0.4) +
    scale_fill_manual(values = c('#ff8b5c','#ffb594', '#ffffff','#b1e1d1','#88d2ba','#5ac2a4','#00b28f'), 
                         labels = c("-2k to -1k", "-1k to 0", "0 to 1k", "1k to 2k", "2k to 3k", "3k to 4k", "4k to 14k"),
                         name = "Change in Housing") +
  coord_equal() +
   labs(x = NULL, 
        y = NULL, 
        title = "New Housing Concentrated Downtown",
        subtitle = "Change in Housing: 2000 - 2017",
        caption = "Data from American Community Survey") +
    theme_minimal() +
    theme(axis.line = element_blank(),
    axis.text.x = element_blank(),
